  ...
  const buscarKardex = async () => {
    ...
    try {
      ...
      const tirillasCargadas = data.map(r => ({
        fecha: r.fecha_recepcion,
        descripcion: r.nombre_insumo || r.factura || r.lote || r.casa_comercial || "Registro kardex",
        detalle: r,
      }));
      ...
      <div className="tirillas-wrapper">
        ...
        <KardexListaTirillas
          ...
          onActualizarPagoFactura={async ({ factura, pagado }) => {
            try {
              const res = await fetch(`${import.meta.env.VITE_API_URL}/kardex/pagado/${factura}`, { 
                method: "PUT",
                headers: {
              ...
              const recargar = await fetch(`${import.meta.env.VITE_API_URL}/kardex?id_sede=${idSede}`, { 
                headers: { "Authorization": `Bearer ${token}` }
              });
